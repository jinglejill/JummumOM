...
/** 返回扫描到的蓝牙 设备列表 Return to the list of scanned Bluetooth devices
 *  因为蓝牙模块一次返回一个设备，所以该方法会调用多次
Because the Bluetooth module returns one device at a time, this method is called multiple times.
 */
- (void)printerManager:(GprinterBluetooth *)manager perpherals:(NSArray<CBPeripheral *> *)perpherals isTimeout:(BOOL)isTimeout;

/** 扫描蓝牙设备失败 Scanning Bluetooth device failed
 *
 */
- (void)printerManager:(GprinterBluetooth *)manager scanError:(ScanError)error;

/**
 *  连接蓝牙外设完成 Connect the Bluetooth peripheral to complete
 *
 *  @param manager   管理中心
 *  @param perpheral 蓝牙外设
 *  @param error     错误
 */
- (void)printerManager:(GprinterBluetooth *)manager completeConnectPerpheral:(CBPeripheral *)perpheral error:(NSError *)error;
 ...
 *  @param error      错误信息
 */
- (void)printerManager:(GprinterBluetooth *)manager disConnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error;

@end


@interface GprinterBluetooth : NSObject
...
/**
 *  蓝牙外设是否已连接 Is the Bluetooth peripheral connected?
 *
 *  @return YES/NO
 */
- (BOOL)isConnected;
...
/**
 *  打印自己组装的数据Print your own assembled data
 *
 *  @param data   数据
 *  @param result 结果
 */
...
/**
 *  查询蓝牙打印机状态 * Query Bluetooth printer status
 *  @param n   1~2
 *
 */
-(void)SearchBlueToothPrinterStatus:(int)n;

//查询蓝牙机型Query Bluetooth model
-(void)SearchBlueToothPrinterModelNum;

//从打印机获取到的数据Data obtained from the printer
...
@property(nonatomic,assign)int isNum;
